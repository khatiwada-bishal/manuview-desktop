  ...
  return (
    <div className="flex-1 overflow-y-auto bg-white p-6 sm:p-10 text-[#111827]">
      <div className="max-w-5xl mx-auto space-y-8">
        ...
        {/* Input Form */}
        <form onSubmit={handlePredict} className="p-6 rounded-2xl bg-[#F9FAFB] border border-[#E5E7EB] space-y-4 shadow-xs">
          ...
          <div className="space-y-1">
            ...
            <input
              ...
              className="w-full px-3.5 py-2.5 rounded-xl border border-[#E5E7EB] bg-white text-xs sm:text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500"
            />
          </div>

          <div className="space-y-1">
            <label className="text-xs font-semibold text-neutral-700">Manuscript Abstract</label>
            <textarea
              rows={4}
              value={abstract}
              onChange={(e) => setAbstract(e.target.value)}
              placeholder="Paste the manuscript abstract or key summary..."
              className="w-full px-3.5 py-2.5 rounded-xl border border-[#E5E7EB] bg-white text-xs sm:text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 resize-none"
            />
          </div>
          ...
          <div className="space-y-6 animate-in fade-in duration-300">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div className="p-5 rounded-2xl border-2 border-amber-200 bg-amber-50/40 space-y-3">
                ...
                <div>
                  <h3 className="font-bold text-sm text-[#0F172A]">{results.reach.name}</h3>
                  <div className="text-xs text-neutral-500 mt-0.5">{results.reach.publisher}</div>
                </div>
                <div className="space-y-1.5 pt-2 border-t border-amber-200/60 text-xs text-neutral-600">
                  <div className="flex justify-between">
                    <span>Acceptance Rate:</span>
                    <span className="font-medium text-neutral-800">{results.reach.acceptanceRate}</span>
                  </div>
                  <div className="flex justify-between">
                    <span>Turnaround:</span>
                    <span className="font-medium text-neutral-800">{results.reach.reviewSpeed}</span>
                  </div>
                  <div className="flex justify-between">
                    <span>Access Model:</span>
                    <span className="font-medium text-neutral-800">{results.reach.openAccess}</span>
                  </div>
                </div>
              </div>

              {/* Realistic Target */}
              <div className="p-5 rounded-2xl border-2 border-emerald-300 bg-emerald-50/50 space-y-3 shadow-xs">
                <div className="flex items-center justify-between">
                ...
                <div>
                  <h3 className="font-bold text-sm text-[#0F172A]">{results.realistic.name}</h3>
                  <div className="text-xs text-neutral-500 mt-0.5">{results.realistic.publisher}</div>
                </div>
                <div className="space-y-1.5 pt-2 border-t border-emerald-200/60 text-xs text-neutral-600">
                  <div className="flex justify-between">
                    <span>Acceptance Rate:</span>
                    <span className="font-medium text-neutral-800">{results.realistic.acceptanceRate}</span>
                  </div>
                  <div className="flex justify-between">
                    <span>Turnaround:</span>
                    <span className="font-medium text-neutral-800">{results.realistic.reviewSpeed}</span>
                  </div>
                  <div className="flex justify-between">
                    <span>Access Model:</span>
                    <span className="font-medium text-neutral-800">{results.realistic.openAccess}</span>
                  </div>
                </div>
              </div>
              ...
              <div className="p-5 rounded-2xl border-2 border-blue-200 bg-blue-50/40 space-y-3">
                ...
                <div>
                  <h3 className="font-bold text-sm text-[#0F172A]">{results.fallback.name}</h3>
                  <div className="text-xs text-neutral-500 mt-0.5">{results.fallback.publisher}</div>
                </div>
                <div className="space-y-1.5 pt-2 border-t border-blue-200/60 text-xs text-neutral-600">
                  <div className="flex justify-between">
                    <span>Acceptance Rate:</span>
                    <span className="font-medium text-neutral-800">{results.fallback.acceptanceRate}</span>
                  </div>
                  <div className="flex justify-between">
                    <span>Turnaround:</span>
                    <span className="font-medium text-neutral-800">{results.fallback.reviewSpeed}</span>
                  </div>
                  <div className="flex justify-between">
                    <span>Access Model:</span>
                    <span className="font-medium text-neutral-800">{results.fallback.openAccess}</span>
                  </div>
                </div>
              </div>
            </div>
                    ...
                    {results.allMatches.slice(0, 10).map((match, idx) => (
                      <tr key={idx} className="hover:bg-neutral-50/70 transition">
                        <td className="px-4 py-3">
                          <div className="font-semibold text-neutral-900">{match.journal.name}</div>
                          <div className="text-[11px] text-neutral-400">{match.journal.publisher}</div>
                        </td>
                        <td className="px-4 py-3">
                          <span className="inline-block px-2 py-0.5 rounded-full font-bold bg-emerald-50 text-emerald-700 border border-emerald-200 text-[11px]">
                            {match.matchScore}%
                          </span>
                        </td>
