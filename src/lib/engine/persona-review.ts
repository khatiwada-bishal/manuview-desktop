...
export function computePanelConsensus(
  ...
  if (consensusLevel === "unanimous") {
    borderlineDiagnosis =
      rejectFamily === total
        ? "The simulated reviewer perspectives are unanimous in recommending extensive structural revisions or alternate venue targeting prior to submission."
        : "The simulated reviewer perspectives concur that the manuscript is suitable for peer review following targeted revisions.";
    ...
    if (editorIsReject && !methodsIsReject) {
      borderlineDiagnosis =
        "Simulated perspectives split on editorial significance and journal remit rather than methodology — your borderline hazard is venue framing and scope alignment.";
    } else if (methodsIsReject && !editorIsReject) {
      borderlineDiagnosis =
        "Simulated perspectives split on experimental controls and empirical proof rather than conceptual interest — your borderline hazard is methodological rigor.";
    } else {
      borderlineDiagnosis =
        "Simulated perspectives show divergent reviewer opinions across evaluation tracks, indicating a borderline submission requiring thorough author rebuttal preparation.";
    }
  } else {
    borderlineDiagnosis =
      rejectFamily > reviseFamily
        ? "A majority of simulated reviewer tracks advise rejection or extensive restructuring, indicating substantial evidentiary or scope hurdles."
        : "A majority of simulated reviewer tracks support proceeding to peer review after addressing specific methodological caveats.";
  }
